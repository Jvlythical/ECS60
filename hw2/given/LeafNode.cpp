...
BTreeNode* LeafNode::tryBorrowing() {
	...
	if(!this->isHalfFull()) {
		...
		LeafNode *left_leaf = static_cast<LeafNode *> (this->getLeftSibling());
		LeafNode *right_leaf =  static_cast<LeafNode *>(this->getRightSibling());
...
BTreeNode* LeafNode::merge(bool direction) {
	BTreeNode* new_node = NULL;
	int leaf_count = count;	
	// 1 = Merge left
	// 0 = Merge right

	if(direction) {
		
		for(int i = 0; i < leaf_count; i++) this->moveChildLeft();

		new_node = this->leftSibling;
		this->leftSibling->setRightSibling(this->rightSibling);
	} else {

		for(int i = 0; i < leaf_count; i++) this->moveChildRight();
...
BTreeNode* LeafNode::moveChildLeft() {
	...
	InternalNode* parent_node = dynamic_cast<InternalNode*>(this->parent);
	
	if(parent_node != NULL) {
		for(int i = 0; i < parent_node->getCount(); i++) {
			BTreeNode* c = parent_node->getChildren()[i];

			parent_node->getKeys()[i] = c->getMinimum();
		}
	}
