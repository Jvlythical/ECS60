...

{
		...
		cout << "Range: " << x << " - " << y << endl;
		if(x < y) printRangeHelper(x, y, root);
		else printRangeHelper(y, x, root);
		cout << endl;
}

template <class Comparable>
void BinarySearchTree<Comparable>::printRangeHelper(const Comparable &x, const Comparable &y, BinaryNode<Comparable> *n) const {
    if(n == NULL) return;

    if(x < n->element) printRangeHelper(x, y, n->left);
    if(x <= n->element && y >= n->element) cout << n->element << " ";
   	if(y > n->element) printRangeHelper(x, y, n->right);
}
