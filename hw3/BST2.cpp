...

{
    ...
    printLeft(x, root->left);
    printRight(y, root->right);
}

template <class Comparable>
void BinarySearchTree<Comparable>::printLeft(const Comparable &x, BinaryNode<Comparable> *n) const {
    if(n == NULL) return;
   
    cout << n->element << endl;
    
    if(x < n->right->element) printRight(x, n->right);
    if(x < n->left->element) printLeft(x, n->left);
     
    return;
}

template <class Comparable>
void BinarySearchTree<Comparable>::printRight(const Comparable &y, BinaryNode<Comparable> *n) const {
    if(n == NULL) return;
    
    cout << n->element << endl;
    
    if(y > n->right->element) printRight(y, n->right);
    if(y > n->left->element) printLeft(y, n->left);
